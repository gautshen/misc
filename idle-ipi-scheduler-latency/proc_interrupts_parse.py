...
def delta_interrupts(before_dict, after_dict):
    ret_dict = {}
    for cpu in before_dict.keys():
        ret_dict[cpu] = {}
        for irq_nr in before_dict[cpu].keys():
            try:
...
for cpu in cpus:
    #print "CPU %d :" %(cpu)
    total_irqs=0
    for irq_nr in delta_dct[cpu].keys():
        if (delta_dct[cpu][irq_nr] != 0):
            total_irqs = total_irqs + delta_dct[cpu][irq_nr]
            print "CPU %d: %s [%s] = %d times" %(cpu, irq_nr.split(',')[0], irq_names[irq_nr], delta_dct[cpu][irq_nr])
    print ("CPU %d: Total IRQs = %d" %(cpu, total_irqs))
